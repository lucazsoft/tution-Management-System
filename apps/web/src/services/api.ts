...
export const api = {
  ...
  teacher: {
    ...
    updateTopicLog: async (syllabusId: string, payload: { topicId: string; status: 'IN_PROGRESS' | 'COMPLETED' | 'LEFT'; notes?: string; logDate?: string }) =>
      request<{ message: string; log: { id: string; topicId: string; status: 'IN_PROGRESS' | 'COMPLETED' | 'LEFT'; notes: string | null; logDate: string; updatedAt?: string }; topic?: { id: string; status: 'IN_PROGRESS' | 'COMPLETED' | 'LEFT' }; chapter?: { id: string; status: 'IN_PROGRESS' | 'COMPLETED' | 'LEFT' } }>(`/teacher/syllabus/${syllabusId}/topic-log`, { method: 'POST', body: JSON.stringify(payload) }),
  ...
  branchAdmin: {
    ...
    getCertificateOptions: async () => request<{ templates: Array<{ id: string; name: string; type: string; status: string; version: number; updatedAt: string; layoutConfig: { renderMode?: string; html?: string; theme?: 'CLASSIC' | 'MODERN' | 'ACADEMIC'; title?: string; presentationLine?: string; achievementLine?: string; signatoryName?: string; signatoryTitle?: string; sourceFile?: { name: string; mimeType: string } } }>; students: Array<{ studentId: string; studentName: string; gradeName: string; classId: string; className: string; branchId: string; branchName: string }> }>('/certificates/options'),
    ...
    updateCertificateTemplate: async (templateId: string, payload: { name: string; type: string; layoutConfig: unknown }) => request<{ message: string }>(`/certificates/templates/${encodeURIComponent(templateId)}`, { method: 'PATCH', body: JSON.stringify(payload) }),
